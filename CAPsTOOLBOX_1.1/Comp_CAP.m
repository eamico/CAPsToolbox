...
for pp = 1:length(TH) 
    ...
    for j = 1:Liu_NCap
        V.fname = fullfile(Dir,['CAP_' num2str(j) '_' State '_' num2str(thr) 'Percent.nii']);
        V.dt=[16,0];
        S_CAP{j} = reshape( CAP{j},V.dim);
        Image = spm_write_vol(V,S_CAP{j});     
    end
    V.fname = fullfile(Dir,['MeanAct_' State '_' num2str(thr) 'Percent.nii']);
    V.dt=[16,0];
    ...
    save(fullfile(Dir,['Cap_par_' State '_' num2str(thr) '.mat']),'Cap_par');
    save(fullfile(Dir,['CAP_' State '_' num2str(thr) '.mat']),'S_CAP');
end
